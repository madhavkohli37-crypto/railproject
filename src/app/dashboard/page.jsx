...
function PassengerView({ user }) {
  ...
  const handleFileChange = (e) => {
    ...
    if (!file.type.startsWith('image/')) {
      setUploadMsg({ type: 'error', text: 'Please select a valid image file (JPG, PNG, WEBP).' });
      return;
    }

    if (file.size > 5 * 1024 * 1024) {
      setUploadMsg({ type: 'error', text: 'Image size must be under 5MB.' });
      return;
    }
  ...
  const handleUploadAadhaar = async (e) => {
    e.preventDefault();
    if (!aadharNumber || !aadharImage) {
      setUploadMsg({ type: 'error', text: 'Please provide both Aadhaar number and card image.' });
      return;
    }

    const cleanNum = aadharNumber.replace(/\D/g, '');
    if (cleanNum.length !== 12) {
      setUploadMsg({ type: 'error', text: 'Aadhaar number must be exactly 12 digits.' });
      return;
    }
    ...
    try {
      const res = await api.post('/passenger/aadhar-upload', {
        aadhar_number: cleanNum,
      ...
      setUploadMsg({ type: 'error', text: err.response?.data?.error || 'Failed to upload Aadhaar document.' });
    } finally {
      setUploading(false);
    }
  };
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <div className="bg-gradient-to-r from-[#1a3a6b] to-[#2563eb] dark:from-[#0f2347] dark:to-[#1a3a6b] text-white rounded-2xl p-8 relative overflow-hidden">
        ...
        <div className="mt-6">
          <Link href="/book" className="bg-orange-500 hover:bg-orange-600 text-white font-semibold px-6 py-2.5 rounded-lg text-sm shadow-sm">
            🧳 Request Assistance
          </Link>
        </div>
      </div>
          ...
          <div>
            ...
            <div className="space-y-3 text-sm">
              <div>
                ...
                <p className="font-semibold dark:text-white text-base">{profile?.name || user?.name}</p>
              </div>
              <div>
                <p className="text-gray-400 text-xs">Email Address</p>
                <p className="font-semibold dark:text-white">{profile?.email || user?.email}</p>
              </div>
              <div>
                <p className="text-gray-400 text-xs">Phone Number</p>
                <p className="font-semibold dark:text-white">{profile?.phone || user?.phone || 'Not provided'}</p>
              </div>
              <div>
                <p className="text-gray-400 text-xs">ID Verification Status</p>
                <div className="mt-1">
                  {verificationStatus === 'VERIFIED' && (
                    <span className="bg-green-100 text-green-800 dark:bg-green-900/40 dark:text-green-300 px-3 py-1 rounded-full text-xs font-bold inline-flex items-center gap-1">
                      🛡️ Verified Passenger
                    </span>
                  )}
                  {verificationStatus === 'PENDING' && (
                    <span className="bg-yellow-100 text-yellow-800 dark:bg-yellow-900/40 dark:text-yellow-300 px-3 py-1 rounded-full text-xs font-bold inline-flex items-center gap-1">
                      ⏳ Verification Pending Review
                    </span>
                  )}
                  {verificationStatus === 'REJECTED' && (
                    <span className="bg-red-100 text-red-800 dark:bg-red-900/40 dark:text-red-300 px-3 py-1 rounded-full text-xs font-bold inline-flex items-center gap-1">
                      ❌ Verification Rejected
                    </span>
                  )}
                  {verificationStatus === 'UNVERIFIED' && (
                    <span className="bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300 px-3 py-1 rounded-full text-xs font-bold inline-flex items-center gap-1">
                      ⚠️ Not Verified
                    </span>
                  )}
                </div>
        ...
        <div className="card lg:col-span-2">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
              🪪 Government ID (Aadhaar Card) Verification
            </h2>
            {profile?.aadhar_image && (
              <button
                onClick={() => setViewDocModal(true)}
                className="text-xs bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 font-semibold px-3 py-1.5 rounded-lg hover:bg-blue-100"
              >
          ...
          {/* VERIFIED STATE */}
          {verificationStatus === 'VERIFIED' && !showUploadForm && (
            <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-900/40 rounded-xl p-5">
              <div className="flex items-start gap-4">
                <div className="w-12 h-12 rounded-full bg-green-500 text-white flex items-center justify-center text-2xl font-bold shrink-0">
                  🛡️
                </div>
                <div className="space-y-1">
                  <h3 className="font-bold text-green-900 dark:text-green-300 text-base">
                    Aadhaar Card Verified
                  </h3>
                  <p className="text-xs text-green-700 dark:text-green-400">
                    Your government ID has been reviewed and verified by Indian Railways station administration.
                  </p>
                  <div className="pt-2 flex items-center gap-4 text-xs font-semibold text-green-900 dark:text-green-300">
                    <span>Aadhaar: XXXX-XXXX-{profile?.aadhar_number?.slice(-4) || '****'}</span>
                    {profile?.verification_reviewed_at && (
                      <span>Verified on: {new Date(profile.verification_reviewed_at).toLocaleDateString()}</span>
                    )}
                  </div>
                </div>
              </div>
          ...
          {/* PENDING STATE */}
          {verificationStatus === 'PENDING' && !showUploadForm && (
            <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-900/40 rounded-xl p-5">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div className="flex items-start gap-3">
                  <div className="w-10 h-10 rounded-full bg-yellow-500 text-white flex items-center justify-center text-xl shrink-0">
                    ⏳
                  </div>
                  <div>
                    <h3 className="font-bold text-yellow-900 dark:text-yellow-300 text-base">
                      Verification Under Review
                    </h3>
                    <p className="text-xs text-yellow-700 dark:text-yellow-400 mt-0.5">
                      Submitted Aadhaar No: <strong>XXXX-XXXX-{profile?.aadhar_number?.slice(-4) || '****'}</strong>
                    </p>
                    <p className="text-[11px] text-yellow-600 dark:text-yellow-500 mt-1">
                      Submitted on: {profile?.verification_submitted_at ? new Date(profile.verification_submitted_at).toLocaleString() : 'Recently'}
                    </p>
                  </div>
                </div>
                <button
                  onClick={() => {
                    ...
                    setAadharNumber(profile?.aadhar_number || '');
                  }}
                  className="text-xs font-semibold bg-white dark:bg-gray-800 text-yellow-800 dark:text-yellow-300 border border-yellow-300 dark:border-yellow-700 px-3.5 py-2 rounded-lg hover:bg-yellow-100 shrink-0"
                >
                  ✏️ Update Document
                </button>
              </div>
          ...
          {/* REJECTED STATE */}
          {verificationStatus === 'REJECTED' && !showUploadForm && (
            <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-900/40 rounded-xl p-5">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div className="flex items-start gap-3">
                  <div className="w-10 h-10 rounded-full bg-red-500 text-white flex items-center justify-center text-xl shrink-0">
                    ❌
                  </div>
                  <div>
                    ...
                    <p className="text-xs text-red-700 dark:text-red-400 mt-0.5">
                      Reason: <strong>{profile?.verification_notes || 'Uploaded document was unreadable or incomplete.'}</strong>
                    </p>
                ...
                <button
                  ...
                  className="text-xs font-semibold bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg shrink-0"
                >
                  🔄 Re-submit Aadhaar
                </button>
              </div>
          ...
          {/* UPLOAD FORM (UNVERIFIED or when editing) */}
          {(verificationStatus === 'UNVERIFIED' || showUploadForm) && (
            <form onSubmit={handleUploadAadhaar} className="space-y-4">
              <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-900/30 p-3 rounded-lg text-xs text-blue-800 dark:text-blue-300">
                🔒 Your Aadhaar card is encrypted and stored securely for identity verification by station authorities.
              </div>

              <div>
                <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">
                  12-Digit Aadhaar Number <span className="text-red-400">*</span>
                </label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">
                  Aadhaar Card Front Image / Scan <span className="text-red-400">*</span>
                </label>
                <input
                  type="file"
                  accept="image/*"
                  required={!profile?.aadhar_image}
              ...
              {previewUrl && (
                <div className="mt-3 p-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl border border-gray-200 dark:border-gray-600">
                  <p className="text-xs font-semibold text-gray-500 dark:text-gray-300 mb-2">📷 Selected Aadhaar Preview:</p>
                  <img src={previewUrl} alt="Aadhaar Preview" className="max-h-48 rounded-lg shadow-sm object-contain border border-gray-200 dark:border-gray-600" />
                </div>
              )}
              ...
              <div className="flex items-center gap-3 pt-2">
                ...
                >
                  {uploading ? 'Uploading Document...' : '📤 Submit Aadhaar for Verification'}
                </button>
                {showUploadForm && (
                  <button
                    type="button"
      ...
      {/* Document View Modal for Passenger */}
      {viewDocModal && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4 animate-fade-in">
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 w-full max-w-xl shadow-2xl relative space-y-4">
            ...
            <div className="text-sm space-y-1">
              <p className="dark:text-gray-300"><strong>Passenger:</strong> {profile?.name}</p>
              <p className="dark:text-gray-300"><strong>Aadhaar Number:</strong> {profile?.aadhar_number}</p>
              <p className="dark:text-gray-300"><strong>Status:</strong> {verificationStatus}</p>
            </div>

            <div className="bg-gray-900 rounded-xl p-2 flex items-center justify-center min-h-[250px] overflow-hidden border border-gray-700">
              {profile?.aadhar_image ? (
                <img src={profile.aadhar_image} alt="Aadhaar Document" className="max-h-[60vh] object-contain rounded" />
              ) : (
                <p className="text-gray-400 text-sm">No document image found.</p>
              )}
            </div>
...
function AdminView({ currentUser }) {
  ...
  // Passenger ID Verification modal
  ...
  const handleVerifyPassenger = async (userId, action) => {
    let notes = '';
    if (action === 'reject') {
      notes = prompt('Enter reason for rejecting Aadhaar ID (optional):') || 'Uploaded Aadhaar card image was unreadable or incomplete.';
    }
    setVerifyLoadingId(userId);
    try {
      await api.patch(`/admin/verify-passenger/${userId}`, { action, notes });
      setSelectedUserModal(null);
      fetchData();
    } catch (err) {
      alert(err.response?.data?.error || `Failed to ${action} passenger ID`);
    } finally {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
        ...
        <div className="card overflow-hidden p-0">
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left">
              <thead className="text-xs text-gray-500 dark:text-gray-400 uppercase bg-gray-50 dark:bg-gray-700/50">
                <tr>
                  {['User ID','Name','Email','Phone','Aadhaar Status','Aadhaar Card','Actions'].map(h => <th key={h} className="px-4 py-3">{h}</th>)}
                </tr>
              </thead>
              <tbody>
                {loading ? [...Array(3)].map((_, i) => (
                  ...
                  <tr><td colSpan="7" className="text-center py-10 text-gray-500">No passengers yet.</td></tr>
                ) : data.users?.map(u => (
                  <tr key={u.id} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/30">
                    ...
                    <td className="px-4 py-3">
                      {u.verification_status === 'VERIFIED' && (
                        <span className="bg-green-100 text-green-800 dark:bg-green-900/40 dark:text-green-300 text-xs font-bold px-2.5 py-1 rounded-full">🛡️ VERIFIED</span>
                      )}
                      {u.verification_status === 'PENDING' && (
                        <span className="bg-yellow-100 text-yellow-800 dark:bg-yellow-900/40 dark:text-yellow-300 text-xs font-bold px-2.5 py-1 rounded-full">⏳ PENDING REVIEW</span>
                      )}
                      {u.verification_status === 'REJECTED' && (
                        <span className="bg-red-100 text-red-800 dark:bg-red-900/40 dark:text-red-300 text-xs font-bold px-2.5 py-1 rounded-full">❌ REJECTED</span>
                      )}
                      {(!u.verification_status || u.verification_status === 'UNVERIFIED') && (
                        <span className="bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-400 text-xs font-bold px-2.5 py-1 rounded-full">NOT SUBMITTED</span>
                      )}
                    </td>
                    <td className="px-4 py-3">
                      {u.aadhar_image ? (
                        <button
                          ...
                          className="text-xs bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-300 font-semibold px-2.5 py-1 rounded-lg border border-indigo-200 dark:border-indigo-800 hover:bg-indigo-100"
                        >
                          🖼️ View Card
                        </button>
                      ) : (
                    ...
                    <td className="px-4 py-3">
                      {u.aadhar_image && u.verification_status !== 'VERIFIED' ? (
                        <div className="flex gap-1.5">
                          <button
                            onClick={() => handleVerifyPassenger(u.id, 'approve')}
                            disabled={verifyLoadingId === u.id}
                            className="bg-green-600 hover:bg-green-700 text-white text-xs font-bold px-2.5 py-1 rounded-lg disabled:opacity-50"
                          >
                            ✅ Approve
                          </button>
                          <button
                            onClick={() => handleVerifyPassenger(u.id, 'reject')}
                            disabled={verifyLoadingId === u.id}
                            className="bg-red-600 hover:bg-red-700 text-white text-xs font-bold px-2.5 py-1 rounded-lg disabled:opacity-50"
                          >
                            ❌ Reject
                          </button>
                        </div>
      ...
      {/* View Passenger Aadhaar Modal for Admin */}
      {selectedUserModal && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4 animate-fade-in">
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 w-full max-w-xl shadow-2xl space-y-4">
            <div className="flex justify-between items-center border-b pb-3 dark:border-gray-700">
              <div>
                <h3 className="font-bold text-lg dark:text-white flex items-center gap-2">
                  🪪 Passenger Aadhaar Document Verification
                </h3>
                <p className="text-xs text-gray-500">Passenger: {selectedUserModal.name} (U-{selectedUserModal.id})</p>
              </div>
              <button
            ...
            <div className="grid grid-cols-2 gap-3 text-xs bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg">
              <div><span className="text-gray-400 block">Aadhaar Number:</span> <span className="font-bold font-mono text-sm dark:text-white">{selectedUserModal.aadhar_number || 'Not provided'}</span></div>
              <div><span className="text-gray-400 block">Status:</span> <span className="font-bold dark:text-white">{selectedUserModal.verification_status || 'PENDING'}</span></div>
              <div><span className="text-gray-400 block">Email:</span> <span className="font-semibold text-blue-600 dark:text-blue-400">{selectedUserModal.email}</span></div>
              <div><span className="text-gray-400 block">Submitted At:</span> <span className="font-medium dark:text-gray-300">{selectedUserModal.verification_submitted_at ? new Date(selectedUserModal.verification_submitted_at).toLocaleString() : 'N/A'}</span></div>
            </div>

            <div className="bg-gray-900 rounded-xl p-2 flex items-center justify-center min-h-[250px] overflow-hidden border border-gray-700">
              {selectedUserModal.aadhar_image ? (
                <img src={selectedUserModal.aadhar_image} alt="Aadhaar Document" className="max-h-[60vh] object-contain rounded" />
              ) : (
            ...
            <div className="flex justify-between items-center pt-2">
              <div className="flex gap-2">
                <button
                  onClick={() => handleVerifyPassenger(selectedUserModal.id, 'approve')}
                  disabled={verifyLoadingId === selectedUserModal.id}
                  className="bg-green-600 hover:bg-green-700 text-white font-semibold text-xs py-2 px-4 rounded-lg disabled:opacity-50"
                >
                  ✅ Approve ID
                </button>
                <button
                  onClick={() => handleVerifyPassenger(selectedUserModal.id, 'reject')}
                  disabled={verifyLoadingId === selectedUserModal.id}
                  className="bg-red-600 hover:bg-red-700 text-white font-semibold text-xs py-2 px-4 rounded-lg disabled:opacity-50"
                >
                  ❌ Reject ID
                </button>
              </div>
              <button
                onClick={() => setSelectedUserModal(null)}
                className="px-4 py-2 text-xs text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 font-semibold"
              >
                Close
              </button>
            </div>
